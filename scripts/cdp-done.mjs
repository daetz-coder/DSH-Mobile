// CDP: send a tiny task so the agent produces activity, so we can capture a
// live sessionStats projection and confirm its authoritative fields.
...
const r = await send('Runtime.evaluate', {
  expression: `JSON.stringify((() => {
    ...
    ta.focus();
    Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value').set.call(ta, 'Reply with the single word: done');
